...
ModuleAssembly <- function(weightChosen, FDRCutoff, caseName, controlName, 
  ...
  summaryFDRModules1 <- read.csv(paste(outputPrefix, 
    "_summary_FDR_weight_backward_", weightChosen, ".csv", sep=""))
  ...
  for (i in 1:topModuleAssembly) {
    ...
    if (length(indexModuleEnrichPathway) >= 2) {
      ...
      if (length(groupGenes) >= 3) {
        ...
        write.table(edgeList, file=paste(pathwayName, "_edge_list.txt", 
          sep=""), row.names=FALSE, 
        quote=FALSE, sep="\t")
        write.table(nodeList, file=paste(pathwayName, "_node_list.txt", sep=""), row.names=FALSE, 
        quote=FALSE, sep="\t")      
    ...
    system(paste("zip -q module_assembly_edge_node_list.zip ", "GO*.txt ", 
      "KEGG*.txt ", "REACTOME*.txt ", "BIOCARTA*.txt", sep=""))  
